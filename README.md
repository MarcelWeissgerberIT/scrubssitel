...
Existing version-one, version-two and version-three hospitals are migrated without deleting rooms, cash or active patients. They are offered the new tutorial, and can instead continue their existing game. Export a save before replacing a hospital if you want to keep both.
...
The original ensemble and both new full-body sprite sheets were generated through the owner's signed-in **OpenArt** account using GPT Image 2. See [`PROVENANCE.md`](public/assets/PROVENANCE.md) and [`PROVENANCE-V2.md`](public/assets/PROVENANCE-V2.md) for prompts, layout and processing details. The procedural room geometry and comedy machines are interactive Canvas game scenery. Version 2.2 replaced the old sliced bitmap gait with procedurally shaded, articulated character geometry. OpenArt portraits and concept sheets remain the cast reference and directory artwork. Version 2.3 adds code-drawn interactive furniture and children based on that original style. No new OpenArt furniture/turnaround generation is claimed: Chrome was in concurrent use. Bundled fallback announcements use eSpeak NG formant voices (`de` / `en-us`), rendered to AAC. No Apple system-voice recordings are distributed. Texts, source/license notes and verification hashes are in `public/assets/voice/PROVENANCE.md` and `provenance.json`. Fonts ship with their SIL Open Font Licenses.
