...
export class Game {
 constructor({level=1,mode='tutorial',seed=Date.now()}={}){
  this.version=3;this.recruitmentRound=0;this.applicantIds=candidates(0).map(c=>c.id);this.mode=mode;this.level=level;this.rng=seed>>>0||1;this.clock=0;this.day=1;this.cash=50000;
 ...
 hireApplicant(applicantId,free=false){const base=this.applicants().find(c=>c.id===applicantId);if(!base)return {error:'noApplicants'};if(!free&&this.cash<base.hire)return {error:'notEnough'};const s={...base,id:++this.id,applicantId:base.id,fatigue:0,resting:false,roomId:null};this.applicantIds=this.applicantIds.filter(id=>id!==applicantId);this.staff.push(s);if(!free){this.cash-=base.hire;this.construction+=base.hire;this.log('hireDone',s.name);}this.assignStaff();for(const type of Object.keys(ROOMS))this.rebalance(type);this.checkTutorial();return {staff:s};}
 ...
 sell(id){const r=this.room(id);if(!r)return {};if(r.patientId||this.patients.some(p=>p.seatRoom===id||this.contains(r,p)))return {error:'removeBusy'};const s=this.staff.find(s=>s.id===r.staffId);if(s)s.roomId=null;for(const p of this.patients)if(p.targetRoom===id){p.path=this.corridorPath(p,this.room(p.seatRoom)?this.door(this.room(p.seatRoom)):p)||[];p.seatRoom=null;p.seatIndex=null;p.targetRoom=null;p.state=p.path.length?'relocating':'waiting';}this.cash+=Math.round(ROOMS[r.type].cost*.5);this.construction-=Math.round(ROOMS[r.type].cost*.5);this.rooms=this.rooms.filter(r=>r.id!==id);this.assignStaff();return {};}
 ...
 seats(r){if(r.type!=='waiting')return [];const out=[];for(let row=0;row<(r.h>=5?2:1);row++)for(let i=0;i<Math.min(6,Math.floor((r.w-.5)/1.3));i++)out.push({x:r.x+.45+i*1.3,y:r.y+.6+row*1.6});return out;}
 ...
 routePatient(p){const type=this.need(p);const choices=this.rooms.filter(r=>r.type===type&&r.staffId).map(r=>({r,path:this.corridorPath(p,this.door(r))})).filter(v=>v.path!==null).sort((a,b)=>(this.queue(a.r)+(a.r.patientId?1:0))-(this.queue(b.r)+(b.r.patientId?1:0))||a.path.length-b.path.length);if(!choices.length)return;const {r,path}=choices[0],busy=r.patientId||this.queue(r)>0;p.targetRoom=r.id;p.path=path;p.state='travel';if(busy&&!this.reserveSeat(p))p.path=this.standingPoint(r,p).path;}
 ...
 update(dt){if(this.over||this.event||dt<=0||!this.admissionsOpen)return;dt=Math.min(dt,.25);this.clock+=dt;
  ...
  const lounge=Math.max(0,...this.rooms.filter(r=>r.type==='lounge').map(r=>r.level)),toilet=Math.max(0,...this.rooms.filter(r=>r.type==='toilet').map(r=>r.level));const janitors=this.staff.filter(s=>s.role==='janitor'&&!s.resting).reduce((n,s)=>n+s.skill,0);
  this.cleanliness=clamp(this.cleanliness+dt*(janitors*.24-this.patients.length*.007-.035),0,100);
  for(const s of this.staff){const r=this.room(s.roomId);const busy=s.role==='janitor'||!!r?.patientId||(r?.type==='lab'&&this.project);if(s.resting){s.fatigue=clamp(s.fatigue-dt*(lounge?2.3*(1+(lounge-1)*.3):.65),0,100);if(s.fatigue<=28)s.resting=false;}else{s.fatigue=clamp(s.fatigue+dt*(busy?.32*(s.fatigueRate||1):- .15),0,100);if(s.fatigue>=86&&!r?.patientId)s.resting=true;}}
  this.assignStaff();this.modifiers=this.modifiers.filter(m=>m.endsAt>this.clock);
  for(const r of this.rooms){r.condition=clamp(r.condition+dt*(janitors*.1-(r.patientId?.055:.006)),20,100);const s=this.staff.find(s=>s.id===r.staffId);if(!s||s.resting)continue;if(!r.patientId){const p=this.patients.find(p=>p.targetRoom===r.id&&['queue','seated','seatTravel','travel'].includes(p.state));if(p){r.patientId=p.id;r.progress=0;p.state='inside';this.recordEvent(p,'serviceStarted',r);p.path=[...(this.corridorPath(p,this.door(r))||[]),...this.enterPath(r,this.servicePoint(r))];p.seatRoom=null;p.seatIndex=null;}}
   ...
   if(r.progress>=ROOMS[r.type].time){r.progress=0;p.state='roomExit';p.path=this.exitPath(r,p);
    if(p.stage==='reception'){p.registered=true;this.recordEvent(p,'registered',r);p.stage='diagnosis';}
    else if(p.stage==='diagnosis'){const fee=this.diagnosisFee();this.cash+=fee;this.income+=fee;this.recordEvent(p,'diagnosed',r,fee);p.stage='treatment';}
  ...
  for(const p of this.patients){if(p.state==='roomExit'){this.move(p,dt);if(!p.path.length){const r=this.room(p.targetRoom);if(r){r.patientId=null;r.progress=0;}p.targetRoom=null;if(p.stage==='exit')this.leave(p);else p.state='waiting';}continue;}if(p.state==='exit'){this.move(p,dt);continue;}
   p.patience=clamp(p.patience-dt*(p.state==='service'?.015:.28)*(p.state==='seated'?Math.max(.2,.35-((this.room(p.seatRoom)?.level||1)-1)*.05):1)*(toilet?(.72-(toilet-1)*.1):1)*(this.completed.includes('patience')?.65:1)*(1+(100-this.cleanliness)/100),0,100);
   if(p.patience<=0&&p.state!=='service'&&p.state!=='inside'){this.leave(p,true);continue;}
   if(p.state==='waiting')this.routePatient(p);
   if(['travel','inside','seatTravel','relocating'].includes(p.state)){this.move(p,dt);if(!p.path.length)p.state={inside:'service',travel:'queue',seatTravel:'seated',relocating:'waiting'}[p.state];}
 ...
 static restore(data){
  ...
  if(!data||data.version!==3||!['tutorial','campaign','sandbox'].includes(data.mode)||![1,2,3].includes(data.level))throw Error('Invalid save');
  ...
  for(const p of data.patients)if(!ILLNESSES.some(i=>i.id===p.illness)||!['waiting','travel','queue','seatTravel','seated','relocating','inside','service','roomExit','exit'].includes(p.state)||!['reception','diagnosis','treatment','exit'].includes(p.stage)||!isNum(p.x)||!isNum(p.y)||!isNum(p.patience)||!Array.isArray(p.path)||p.path.length>500||!p.path.every(v=>isNum(v.x)&&isNum(v.y))||typeof p.name!=='string')throw Error('Invalid patient');
  ...
  for(const r of data.rooms){if(r.staffId!==null&&!data.staff.some(s=>s.id===r.staffId&&s.roomId===r.id))throw Error('Invalid staffing');if(r.patientId!==null&&!data.patients.some(p=>p.id===r.patientId&&p.targetRoom===r.id&&['inside','service','roomExit'].includes(p.state)))throw Error('Invalid occupant');}
  for(const p of data.patients){if(p.name.length>80||p.x<0||p.x>24||p.y<0||p.y>18||p.patience<0||p.patience>100)throw Error('Invalid patient bounds');if(p.targetRoom!==null&&!data.rooms.some(r=>r.id===p.targetRoom))throw Error('Invalid target');if(['inside','service','roomExit'].includes(p.state)&&!data.rooms.some(r=>r.id===p.targetRoom&&r.patientId===p.id))throw Error('Invalid service');}
