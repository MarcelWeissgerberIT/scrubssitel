...
export function validateStaff(g){const seats=new Set();for(const s of g.staff){if(typeof s.manualPlacement!=='boolean'||typeof s.awaitingPlacement!=='boolean'||s.awaitingPlacement&&(!s.manualPlacement||s.roomId!==null||s.state!=='idle'||s.path?.length||s.destination!==null||s.resting||s.breakPending||s.breakRoomId!==null||s.breakSeatIndex!==null||!freeCorridor(g,s))||s.manualPlacement&&!s.awaitingPlacement&&s.role!=='janitor'&&s.roomId===null&&!s.resting&&!(s.state==='travelWork'&&s.destination?.arrival==='idle'&&s.destination.roomId===null))throw Error('Invalid staff placement');
 if(!STAFF_STATES.includes(s.state)||![s.x,s.y,s.breakElapsed,s.nextBreakAt].every(Number.isFinite)||s.breakElapsed<0||s.nextBreakAt<0||!withinGrid(g,s)||!Array.isArray(s.path)||s.path.length>500||s.path.some(p=>!withinGrid(g,p))||typeof s.breakPending!=='boolean'||!Number.isInteger(s.breakCount)||s.breakCount<0||!Number.isInteger(s.patrolIndex)||s.patrolIndex<0)throw Error('Invalid staff journey');
 if(s.destination&&(!withinGrid(g,s.destination)||!STAFF_STATES.includes(s.destination.arrival)||s.destination.roomId!==null&&!g.room(s.destination.roomId)))throw Error('Invalid staff destination');
