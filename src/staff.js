import {workPoint,roomSeats} from './layout.js';
...
export function staffReady(g,s){const r=g.room(s?.roomId);if(!s||!r||s.resting||s.state!=='work'||s.path.length||Array.isArray(r.furniture)&&(!r.ready||r.editing||r.renovating&&!r.patientId))return false;const target=workplace(r);return !!target&&Math.hypot(s.x-target.x,s.y-target.y)<.08;}
...
export function dispatchStaff(g,s){s.breakRoomId=null;s.breakSeatIndex=null;s.resting=false;const r=g.room(s.roomId);if(r&&g.roomReady(r)){if(!walk(g,s,workplace(r),r,'travelWork','work')){s.state='idle';s.destination=null;s.path=[];}return;}
