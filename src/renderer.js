...
import {BASE_GRID,EXPANSIONS} from './expansion.js';
...
export class Renderer{
 ...
 metrics(){const grid=this.game?.grid||BASE_GRID,preview=this.expansionRect(),w=preview?Math.max(grid.w,preview.x+preview.w):grid.w,h=preview?Math.max(grid.h,preview.y+preview.h):grid.h,available=preview&&this.w>=700?Math.max(250,this.w-350):this.w;this.tw=Math.min(available/((w+h)/2+3.8),this.h/((w+h)*.255+2.09))*this.zoom;this.th=this.tw*.51;this.ox=available/2-(w-h)*this.tw/4+this.pan.x;this.oy=this.h/2-(w+h)*this.th/4+28+this.pan.y;}
 drawExpansionPreview(){
  const rect=this.expansionRect();if(!rect)return;const {x,y,w,h}=rect,c=this.ctx;
  this.tileFace(x,y,w,h,.005,'#e2c98738');c.save();c.strokeStyle='#b6925070';c.lineWidth=1;
  for(let row=y+.35;row<y+h;row+=.75){const a=this.project(x,row,.01),b=this.project(x+w,row,.01);c.beginPath();c.moveTo(a.x,a.y);c.lineTo(b.x,b.y);c.stroke();}
  const corners=[this.project(x,y,.012),this.project(x+w,y,.012),this.project(x+w,y+h,.012),this.project(x,y+h,.012)];c.beginPath();corners.forEach((p,i)=>i?c.lineTo(p.x,p.y):c.moveTo(p.x,p.y));c.closePath();c.setLineDash([6,5]);c.strokeStyle='#b28a46';c.lineWidth=1.7;c.stroke();c.setLineDash([]);
  c.restore();
 }
 drawExpansionLabel(){const rect=this.expansionRect();if(!rect)return;const {x,y,w,h,spec}=rect,c=this.ctx,point=this.project(x+w/2,y+h/2,.025),text=tr(spec.name,this.lang)+(this.lang==='de'?' · Vorschau':' · Preview');c.save();c.font=`600 ${Math.max(11,Math.min(14,this.tw*.36))}px sans-serif`;const width=c.measureText(text).width+22;c.fillStyle='#fff9e8f2';c.beginPath();c.roundRect(point.x-width/2,point.y-15,width,30,7);c.fill();c.fillStyle='#84632f';c.textAlign='center';c.fillText(text,point.x,point.y+5);c.restore();}
 ...
 draw(game,time,alpha=1){this.initializingActors=this.game!==game;if(this.game!==game){this.setPanMode(false);this.cancelStaffCarry();this.clearPointer();this.animator.reset();this.visualDoors?.clear();this.loungeSeats?.clear();this.staffVisuals.clear();this.lastTime=time;this.drag=null;this.down=null;this.panning=false;}this.game=game;if(this.panMode&&(this.buildType||this.editor||this.carriedStaffId!==null))this.setPanMode(false);if(this.carriedStaffId!==null&&!game.staff.some(s=>s.id===this.carriedStaffId))this.cancelStaffCarry();this.layoutKey=game.rooms.map(r=>r.id).join(",");this.metrics();const c=this.ctx;this.hits=[];c.clearRect(0,0,this.w,this.h);
  ...
  for(let y=0;y<grid.h;y++)for(let x=0;x<grid.w;x++){const corridor=y>=7&&y<=10||x>=11&&x<=14&&y>7;this.tileFace(x,y,1,1,.005,corridor?((x+y)%2?'#e4eeea':'#eff5f0'):((x+y)%2?'#eef2f3':'#f7f8f5'),'#ffffff26');}
  this.box(0,0,grid.w,.15,1.35,'#b6c9ba');this.box(0,0,.15,grid.h,1.35,'#c2d1bf');
  for(let x=2;x<grid.w-1;x+=4){this.box(x,0,.12,.22,1.6,'#d3deca');this.box(x+.3,.06,Math.min(2,grid.w-x-.6),.06,1.12,'#89b2b5',.55);}
  for(let y=3;y<grid.h-1;y+=4)this.box(.06,y,.06,2,1.12,'#98b7b4',.55);
