...
export class Renderer{
 ...
 furniture(r,time=0){const x=r.x,y=r.y,w=r.w,h=r.h;
  if(r.type==='waiting'){for(const seat of this.game.seats(r)){const xx=seat.x+.5,yy=seat.y+.5;this.box(xx-.36,yy-.28,.72,.62,SEAT_HEIGHT-.02,'#d49b68');this.box(xx-.36,yy-.32,.72,.13,.70,'#edb778');this.box(xx-.39,yy-.25,.1,.66,.45,'#bd855b');this.box(xx+.29,yy-.25,.1,.66,.45,'#bd855b');this.tileFace(xx-.28,yy-.22,.56,.46,SEAT_HEIGHT,'#ffe0a4');}this.plant(x+w-.45,y+h-.5);this.box(x+.45,y+h-.95,.85,.55,.36,'#cbb896');for(let i=0;i<3;i++)this.tileFace(x+.5+i*.17,y+h-.85,.3,.33,.38+i*.015,['#a7becb','#db9a9a','#f0d59a'][i]);return;}
  if(['gp','pharmacy','therapy','surgery','lab'].includes(r.type)){this.machine(r,time);return;}
  if(r.type==='reception'){this.box(x+w*.5-.25,y+.38,.5,.5,SEAT_HEIGHT,'#61958a');this.box(x+.5,y+1,w-1,.8,.56,'#d9bb93');this.box(x+w/2,y+1.2,.5,.2,.91,'#427d80',.56);const bell=this.project(x+.95,y+1.45,.64+(r.patientId?Math.abs(Math.sin(time*8))*.1:0));this.ellipse(bell.x,bell.y,this.tw*.16,this.tw*.09,'#ffe097');this.ellipse(bell.x,bell.y-this.tw*.08,this.tw*.09,this.tw*.08,'#d4a347');return;}

  if(r.type==='lounge'){this.box(x+.4,y+.5,w-1,.75,SEAT_HEIGHT,'#c8844a');this.box(x+.4,y+.4,w-1,.25,.70,'#cf8c50');this.box(x+1.3,y+2,1.2,.9,.35,'#e9d7b2');this.box(x+1.65,y+2.2,.2,.2,.52,'#f8f3de',.35);this.box(x+w-1,y+h-1.1,.7,.6,1.1,'#3c6159');this.plant(x+.6,y+h-.6);}
 ...
 person(person,staff=false,time=0){
  ...
  const bubble=person.cured?'♥':person.patience<35?'!':staff&&person.resting?'z':this.selected?.id===person.id&&person.state==='service'?'✚':null;
  if(bubble){const yy=p.y-height-5,size=Math.max(13,this.tw*.32);c.fillStyle='#fffff6ed';c.beginPath();c.roundRect(p.x-size*.65,yy-size,size*1.3,size*1.3,5);c.fill();c.fillStyle=person.patience<35?'#c47455':'#4b927b';c.font=`bold ${size}px sans-serif`;c.textAlign='center';c.fillText(bubble,p.x,yy);c.textAlign='left';}
 ...
 draw(game,time,alpha=1){this.initializingActors=this.game!==game;if(this.game!==game){this.animator.reset();this.loungeSeats?.clear();this.staffVisuals.clear();this.lastTime=time;this.drag=null;this.down=null;this.panning=false;}this.game=game;this.layoutKey=game.rooms.map(r=>r.id).join(",");this.metrics();const c=this.ctx;this.hits=[];c.clearRect(0,0,this.w,this.h);
