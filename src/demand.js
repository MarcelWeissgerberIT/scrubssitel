import {needsPenalty} from './patient-needs.js';
...
const availableWorker=(g,r)=>{const s=g.staff.find(s=>s.id===r.staffId);return !!s&&!s.trainingCourse&&!faultFor(g,r)&&!s.resting&&!s.breakPending&&['work','travelWork'].includes(s.state);};
...
export function patientHappiness(g,p){
 ...
 return clamp(happiness-needsPenalty(p),0,100);
}
...
export function eligibleIllnesses(g){
 const allowed=g.mode==='tutorial'?ILLNESSES.slice(0,2):ILLNESSES.filter(i=>i.room==='pharmacy'||g.mode==='sandbox'||g.level>=(i.room==='therapy'?2:3));
 return allowed.filter(i=>assigned(g,i.room).length);
}
...
export function getDemandStatus(g){
 ...
 const operating=operationStatus(g),limit=Math.min(operating.admissionLimit,clamp(Math.floor(3+2*bottleneck),3,DEMAND.maxActive));
 ...
 const interval=(DEMAND.startInterval+(mature-DEMAND.startInterval)*growth+(1-quality)*8*growth)*scenarioRules(g).arrivalMultiplier;
 let reason=!g.admissionsOpen||g.over||!operating.open?'closed':!reception.length?'noReception':!diagnosis.length?'noDiagnosis':!treatment.length?'noTreatment':null;
...
export function admitPatient(g,illnessId){
 ...
 const rules=scenarioRules(g),weighted=choices.flatMap(i=>Array(i.room==='therapy'?rules.therapyWeight:i.room==='surgery'?rules.surgeryWeight:1).fill(i));
 const illness=illnessId||weighted[Math.floor(g.random()*weighted.length)].id;
