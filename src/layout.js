import {FURNITURE,requirements,rotatedSize,furnitureRect,furniturePorts} from './objects.js';
...
function interactionAt(g,p){return g.ports.find(port=>port.approach&&onSegment(p,port,port.approach));}
function exemptions(g,a,b){return new Set(g.ports.filter(p=>p.approach&&onSegment(a,p,p.approach)&&onSegment(b,p,p.approach)).map(p=>p.solidId));}
...
export function segmentBlocked(r,a,b,{allowInteraction=true}={}){if(!a||!b||![a.x,a.y,b.x,b.y].every(Number.isFinite))return true;const g=geometry(r);return rawBlocked(r,g,a,b,allowInteraction?exemptions(g,a,b):new Set());}
...
export function furnitureBlocked(r,a,b,{allowInteraction=true}={}){if(!a||!b||![a.x,a.y,b.x,b.y].every(Number.isFinite))return true;const g=geometry(r),ignore=allowInteraction?exemptions(g,a,b):new Set();return g.solids.some(rect=>!ignore.has(rect.id)&&segmentRect(a,b,rect));}
...
export function insidePath(r,from,to){
 ...
 if(fromDoor||toDoor){if(segmentBlocked(r,door,inner))return null;const path=interiorPath(r,fromDoor?inner:from,toDoor?inner:to);if(path===null)return null;return [...(fromDoor?[{...inner}]:[]),...path,...(toDoor?[{...door}]:[])];}
 return interiorPath(r,from,to);
}
function interiorPath(r,from,to){
 if(!from||!to||![from.x,from.y,to.x,to.y].every(Number.isFinite))return null;
 const g=geometry(r);if(!segmentBlocked(r,from,to))return same(from,to)?[]:[{x:to.x,y:to.y}];
 const startPort=interactionAt(g,from),endPort=interactionAt(g,to),start=startPort?.approach||from,end=endPort?.approach||to;
 if(startPort&&segmentBlocked(r,from,start)||endPort&&segmentBlocked(r,end,to))return null;
