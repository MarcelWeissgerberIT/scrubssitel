...
function afterFirstCures(){
 ...
 for(const step of GUIDE.slice(0,7)){if(step.room)assert.ok(furnishedRoom(g,step.room,step.rect).room);else if(step.cast)assert.ok(hireAndPlace(g,step.cast).staff);}
...
test('each initial room lesson waits for furnishing and explicit completion of its existing shell',()=>{
 ...
 for(const [index,step] of GUIDE.slice(0,7).entries()){
  if(step.cast){assert.ok(hireAndPlace(g,step.cast).staff);continue;}
...
for(const type of ['lounge','toilet'])test(`unfinished ${type} cannot complete its lesson or trigger the tutorial win`,()=>{
 ...
 assert.ok(hireAndPlace(g,'otto').staff);
...
test('an upgraded unfinished extra room does not satisfy the upgrade lesson',()=>{
 const g=afterFirstCures();for(const type of ['lounge','toilet'])finish(g,shell(g,type));assert.ok(hireAndPlace(g,'otto').staff);
