...
test('a busy clinic reserves unique seats, keeps overflow moving and gives seated patients comfort',()=>{
 const g=setup(),reception=g.rooms.find(r=>r.type==='reception');g.staff.find(s=>s.id===reception.staffId).resting=true;g.staff.find(s=>s.id===reception.staffId).fatigue=85;
 ...
 const employee=g.staff.find(s=>s.id===reception.staffId);employee.resting=false;employee.fatigue=0;tick(g,220);assert.ok(g.cured>0);assert.ok(g.records.some(r=>r.dischargedAt!==null));
});
